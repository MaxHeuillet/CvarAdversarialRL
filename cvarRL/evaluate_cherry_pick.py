...
# action_idx = np.zeros(args.procs)
# budget_counter = np.zeros(40)
...
while dones[0] == False:
    ...
    position_history.append( agent_pos.cpu().detach() )
    transition_history.append( state_dist.cpu().detach() )
    adversary_history.append( adv_dist.probs.cpu().detach() )
    budget_history.append( budgets.cpu().detach() )
...
with gzip.open(
    os.path.join(
        "./results", "cherry_pick_{}_{}_{}.pkl.gz".format(args.model, args.stochasticity, args.seed)
    ),
    "wb",
) as f:
    pkl.dump(position_history, f)
    pkl.dump(transition_history, f)
    pkl.dump(adversary_history, f)
    pkl.dump(budget_history, f)
    pkl.dump(action_history, f)
print("SAVED")
